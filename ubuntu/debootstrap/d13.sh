...
ARCH="amd64"
IMAGE_NAME="debian13"
IMAGE_SIZE="2G"        # 1GB disk → QCOW2 ~450–550 MB
...
echo "debian" > "${WORKDIR}/mnt/etc/hostname"
...
# APT CONFIGURATION
###############################################################################
cat > "${WORKDIR}/mnt/etc/apt/sources.list" << EOF
...
umount -l "${WORKDIR}/mnt/dev" || true
umount -l "${WORKDIR}/mnt/proc" || true
umount -l "${WORKDIR}/mnt/sys" || true
umount -l "${WORKDIR}/mnt/run" || true
umount -l "${WORKDIR}/mnt" || true
losetup -d "${LOOP}"
